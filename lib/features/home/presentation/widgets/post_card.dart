...
class _PostCardState extends State<PostCard> {
              ...
              Expanded(child: FutureBuilder<List<ManoxComment>>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      final c = comments[i];
                      return ListTile(contentPadding: EdgeInsets.zero, title: Text(c.userName, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text(c.body));
                    },
                  );
                },
              )),
  ...
  Future<void> _sendComment(TextEditingController controller, SupabasePostRepository repo) async {
    ...
    try {
      await repo.addComment(widget.data.id, text);
      controller.clear();
  ...
  Future<void> _share() async {
    ...
    if (repo != null && widget.data.isRemote) {
      try { await repo.recordShare(widget.data.id); } catch (_) {}
    }
  ...
  Future<void> _editPost() async {
    ...
    try {
      await repo.updatePost(widget.data.id, controller.text);
      if (mounted) {
  ...
  Future<void> _deletePost() async {
    ...
    try {
      await repo.deletePost(widget.data.id);
      if (mounted) {
  ...
  void _showMonetizationLock() {
    showDialog<void>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Monetization locked'),
        content: const Text('This earning feature unlocks after the required subscription or MANOX monetization eligibility level is reached.'),
          ...
          if (data.imagePath != null) ...[
            ...
            FutureBuilder<String?>(
              ...
              builder: (context, snapshot) {
                ...
                return ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(snapshot.data!, width: double.infinity, height: 240, fit: BoxFit.cover));
              },
            ),
          ],
          const SizedBox(height: 12),
          Row(children: [
            IconButton(key: Key('post-like-${data.id}'), onPressed: _busy ? null : _toggleLike, icon: Icon(_liked ? Icons.favorite : Icons.favorite_border), tooltip: 'Like'),
            Text('$_likes'),
            const SizedBox(width: 8),
            IconButton(key: Key('post-comment-${data.id}'), onPressed: _showComments, icon: const Icon(Icons.comment_outlined), tooltip: 'Comment'),
            Text('$_comments'),
            const SizedBox(width: 8),
