...
class _PostCardState extends State<PostCard> {
  ...
  bool get _isOwner {
    ...
    return widget.data.isRemote && uid != null && widget.data.ownerUserId == uid;
  }
            ...
            child: Column(children: [
              ...
              Expanded(child: FutureBuilder<List<ManoxComment>>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 16), itemCount: comments.length, separatorBuilder: (_, __) => const Divider(), itemBuilder: (_, index) { final c = comments[index]; return ListTile(contentPadding: EdgeInsets.zero, title: Text(c.userName, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text(c.body)); });
                },
              )),
              Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 16), child: Row(children: [
                Expanded(child: TextField(controller: controller, textInputAction: TextInputAction.send, decoration: const InputDecoration(hintText: 'Write a comment...'), onSubmitted: (_) => _sendComment(controller, repo))),
                IconButton(icon: const Icon(Icons.send), onPressed: () => _sendComment(controller, repo)),
              ])),
            ]),
          ),
        ),
  ...
  Future<void> _sendComment(TextEditingController controller, SupabasePostRepository repo) async {
    ...
    try { await repo.addComment(widget.data.id, text); controller.clear(); if (mounted) setState(() => _comments += 1); } catch (e) { if (mounted) _showError(e.toString()); }
  }
  ...
  Future<void> _editPost() async {
    ...
    try { await repo.updatePost(widget.data.id, controller.text); if (mounted) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Post updated.'))); await widget.onChanged?.call(); } } catch (e) { if (mounted) _showError(e.toString()); }
    controller.dispose();
  }
  ...
  Future<void> _deletePost() async {
    ...
    try { await repo.deletePost(widget.data.id); if (mounted) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Post deleted.'))); await widget.onChanged?.call(); } } catch (e) { if (mounted) _showError(e.toString()); }
  }

  void _showMonetizationLock() {
    showDialog<void>(context: context, builder: (context) => AlertDialog(title: const Text('Monetization locked'), content: const Text('This earning feature unlocks after the required subscription or MANOX monetization eligibility level is reached.'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('OK'))]));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Card(key: Key('post-card-${data.id}'), margin: const EdgeInsets.symmetric(vertical: 4), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        IconButton(key: Key('post-like-${data.id}'), onPressed: _busy ? null : _toggleLike, icon: Icon(_liked ? Icons.favorite : Icons.favorite_border), tooltip: 'Like'), Text('$_likes'),
        const SizedBox(width: 8),
        IconButton(key: Key('post-comment-${data.id}'), onPressed: _showComments, icon: const Icon(Icons.comment_outlined), tooltip: 'Comment'), Text('$_comments'),
        const SizedBox(width: 8),
        IconButton(key: Key('post-monetize-${data.id}'), onPressed: _showMonetizationLock, icon: const Icon(Icons.currency_rupee), tooltip: 'Earnings'),
