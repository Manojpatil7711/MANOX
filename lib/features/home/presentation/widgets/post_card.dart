...
class _PostCardState extends State<PostCard> {
  ...
  Future<void> _checkOwner() async {
    ...
    final owner = await r.isOwner(widget.data.ownerId);
    if (mounted) setState(() => _isOwner = owner);
  }

  Future<void> _checkSaved() async {
    final r = SupabasePostRepository();
    final saved = await r.isSaved(widget.data.id);
    if (mounted) setState(() => _saved = saved);
  }

  void _openCreator() {
    if (widget.data.ownerId.isEmpty) return;
    context.push('/profile/${widget.data.ownerId}');
  }
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(_saved ? 'Saved to your collection.' : 'Removed from saved.')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not update saved post: $e')),
        );
      }
    } finally {
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      final r = SupabasePostRepository();
      await r.toggleLike(widget.data.id, _liked);
    } catch (e) {
      if (mounted) {
        setState(() {
          _liked = !_liked;
  ...
  Future<void> _openComments() async {
    ...
    try {
      final r = SupabasePostRepository();
      await r.addComment(widget.data.id, text);
      if (mounted) {
        setState(() => _comments++);
  ...
  Future<void> _reportContent() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Thanks. The post was reported to MANOX Safety.')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _blockCreator() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'You will no longer see this creator’s content. You can manage blocks from Community Safety.',
        ),
        actions: [
          TextButton(
    ...
    try {
      final r = SupabasePostRepository();
      await r.blockUser(widget.data.ownerId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not block creator: $e')),
        );
      }
    }
  }
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) => SafeArea(
        child: Column(
          ...
          children: [
            ListTile(
              leading: Icon(_saved ? Icons.bookmark_remove_outlined : Icons.bookmark_add_outlined),
              title: Text(_saved ? 'Remove from saved' : 'Save post'),
              onTap: () {
        ...
        children: [
          ListTile(
            leading: CircleAvatar(
              backgroundImage: widget.data.ownerAvatarUrl == null
                  ? null
                  : NetworkImage(widget.data.ownerAvatarUrl!),
              ...
              children: [
                Row(
                  children: [
                    ...
                    const Spacer(),
                    IconButton(
                      tooltip: _vibed ? 'Remove vibe' : 'Vibe',
                      onPressed: _toggleVibe,
                      icon: Icon(_vibed ? Icons.bolt : Icons.bolt_outlined),
                    ),
                  ],
                ),
                if (widget.data.description.isNotEmpty)
                  Text(widget.data.description, maxLines: 4, overflow: TextOverflow.ellipsis),
                if (widget.data.isMonetizable)
                  Padding(
