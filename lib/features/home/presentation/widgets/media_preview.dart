...
class ManoxMediaPreview extends StatefulWidget {
  ...
  const ManoxMediaPreview({super.key, required this.url, this.height = 240, this.fit = BoxFit.cover, this.autoPlay = false, this.loop = true, this.fullScreenStyle = false});
...
class _ManoxMediaPreviewState extends State<ManoxMediaPreview> {
  ...
  @override Widget build(BuildContext context){final c=_controller;if(_failed)return SizedBox(height:widget.height,child:const Center(child:Text('Video unavailable')));if(c==null||!c.value.isInitialized)return SizedBox(height:widget.height,child:const Center(child:CircularProgressIndicator()));return GestureDetector(onTap:_togglePlayback,child:SizedBox(height:widget.height,width:double.infinity,child:Stack(alignment:Alignment.center,children:[ClipRRect(borderRadius:widget.fullScreenStyle?BorderRadius.zero:BorderRadius.circular(12),child:SizedBox.expand(child:FittedBox(fit:widget.fit,child:SizedBox(width:c.value.size.width,height:c.value.size.height,child:VideoPlayer(c))))),if(!c.value.isPlaying&&!widget.fullScreenStyle)Container(decoration:const BoxDecoration(shape:BoxShape.circle,color:Colors.black54),padding:const EdgeInsets.all(12),child:const Icon(Icons.play_arrow_rounded,color:Colors.white,size:34))])));}
}
