...
class _LearnPageState extends State<LearnPage> {
  ...
  List<_LearnItem> get _results {
    ...
    filtered.sort((a, b) {
      ...
      return rating != 0 ? rating : b.reviews.compareTo(a.reviews);
    });
    return filtered;
  }

  Future<void> _rate(_LearnItem item) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Your $rating-star rating was recorded.')),
    );
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: ['All', 'Teacher', 'Class'].map((value) => Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(children: [
                    ...
                    const Text('Teachers • Standard 1 to 10', style: TextStyle(fontWeight: FontWeight.w900)),
                  ]),
                  const SizedBox(height: 10),
                  SingleChildScrollView(
                    scrollDirection: Axis.horizontal,
                    child: Row(
                      children: standards.map((value) => Padding(
                        padding: const EdgeInsets.only(right: 6),
                        child: ChoiceChip(
                          label: Text(value == 'All Standards' ? 'All' : 'Std $value'),
                          selected: _standard == value,
                          onSelected: (_) => setState(() => _standard = value),
                        ),
                      )).toList(),
                    ),
                  ),
                  ...
                  SwitchListTile.adaptive(
                    ...
                    title: const Text('Expert teachers only', style: TextStyle(fontWeight: FontWeight.w800)),
                    subtitle: const Text('Verified teachers with strong ratings appear first'),
                    value: _expertOnly,
                    onChanged: (value) => setState(() => _expertOnly = value),
                  ),
                ],
              ),
            ),
          ...
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text('${results.length} recommended results', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
          ]),
          const SizedBox(height: 10),
          if (results.isEmpty)
            const Padding(
              ...
              child: Column(children: [
                ...
                SizedBox(height: 12),
                Text('No matching teachers or classes yet.', style: TextStyle(fontWeight: FontWeight.w700)),
                SizedBox(height: 6),
                Text('Try another subject, standard or teacher filter.', textAlign: TextAlign.center),
              ]),
            )
          else
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  SizedBox(width: 12),
                  Expanded(child: Text('Ratings influence discovery, but relevance and quality signals are also considered. MANOX should validate ratings server-side to reduce fake or duplicate reviews.')),
                ],
              ),
            ),
          ),
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                if (item.verified) const Icon(Icons.verified_rounded, size: 18),
              ]),
              ...
              if (item.type == 'Teacher')
                Padding(
                  ...
                  child: Text('Standard ${item.standard}', style: const TextStyle(fontWeight: FontWeight.w700)),
                ),
              const SizedBox(height: 8),
              Row(children: [
                const Icon(Icons.star_rounded, size: 19),
                const SizedBox(width: 4),
                Text(item.rating.toStringAsFixed(1), style: const TextStyle(fontWeight: FontWeight.w800)),
