...
class _CreatePostPageState extends State<CreatePostPage> {
  ...
  static const _kidsCategories = [
    'Science Experiments','Maths','English','History','Geography','GK',
    'Art & Drawing','Music & Dance','Sports','Coding',
  ];
      ...
      builder: (sheet) => SafeArea(child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(children: [
            ...
            if (!widget.initialBeat) Expanded(child: _sourceTile(sheet, Icons.camera_alt_outlined, 'Photo', () => _pick(video: false, source: ImageSource.camera))),
          ]),
  ...
  Future<void> _publish() async {
    final media = _media; final caption = _captionController.text.trim();
    ...
    try {
      String? mediaPath; var mediaType = 'post';
      if (media != null) {
        final bytes = await media.readAsBytes(); final extension = media.path.split('.').last.toLowerCase();
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 32), children: [
      ...
      Card(child: Column(children: [
        ListTile(leading: const Icon(Icons.tune_rounded), title: const Text('Quick edit', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Trim • Crop • Filter • Text • Audio'), trailing: const Icon(Icons.chevron_right_rounded), onTap: _posting ? null : _openTools),
        const Divider(height: 1),
        ListTile(leading: const Icon(Icons.people_outline_rounded), title: const Text('Audience'), subtitle: Text(_audience), trailing: const Icon(Icons.chevron_right_rounded), onTap: _posting ? null : _showAudience),
        SwitchListTile.adaptive(secondary: const Icon(Icons.comment_outlined), title: const Text('Comments'), value: _comments, onChanged: _posting ? null : (v) => setState(() => _comments = v)),
        if (_isVideo) SwitchListTile.adaptive(secondary: const Icon(Icons.download_outlined), title: const Text('Allow downloads'), value: _downloads, onChanged: _posting ? null : (v) => setState(() => _downloads = v)),
      ])),
      const SizedBox(height: 10),
      ...
      const SizedBox(height: 8),
      TextButton.icon(onPressed: _posting ? null : _openTools, icon: const Icon(Icons.build_circle_outlined), label: const Text('Advanced tools')),
    ])),
  );
  ...
  void _showAudience() {
    showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheet) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      for (final option in ['Everyone', 'Followers', 'Only me']) RadioListTile<String>(title: Text(option), value: option, groupValue: _audience, onChanged: (v) { if (v != null) { setState(() => _audience = v); Navigator.pop(sheet); } }),
