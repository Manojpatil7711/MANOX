...
class _CreatePostPageState extends State<CreatePostPage> {
  ...
  @override void initState() { super.initState(); _isVideo = widget.initialBeat; _isBeat = widget.initialBeat; }
  @override void dispose() { _captionController.dispose(); super.dispose(); }
      ...
      builder: (sheet) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Align(alignment: Alignment.centerLeft, child: Text(widget.initialBeat ? 'Upload a BEAT' : 'Create with media', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800))),
            ...
            SizedBox(width: double.infinity, child: OutlinedButton.icon(
              onPressed: () { Navigator.pop(sheet); _pickMedia(video: true, source: ImageSource.camera); },
  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      final visibility = _kidsContent ? 'public' : switch (_audience) { 'Followers' => 'followers', 'Only me' => 'private', _ => 'public' };
      await _repository.createPost(text: caption, imagePath: mediaPath, mediaType: mediaType, visibility: visibility, audienceCategory: _kidsContent ? 'kids_15_plus' : 'general', kidsCategory: _kidsContent ? _kidsCategory : null, allowComments: _comments, allowDownloads: _kidsContent ? false : (_isVideo ? _downloads : false));
  ...
  @override Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      ...
      actions: [Padding(padding: const EdgeInsets.only(right: 10), child: FilledButton.icon(
        onPressed: _posting ? null : _publish,
        icon: const Icon(Icons.publish_rounded, size: 18),
        label: _posting ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Publish'),
      ))],
    ),
    body: SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 32), children: [
      ...
      TextField(controller: _captionController, maxLength: 2200, maxLines: 5, minLines: 2, textCapitalization: TextCapitalization.sentences, decoration: InputDecoration(hintText: widget.initialBeat ? 'Write a BEAT caption…' : 'Add a caption…', alignLabelWithHint: true, border: const OutlineInputBorder())),
      const SizedBox(height: 8),
      if (_isVideo && !widget.initialBeat) Card(child: SwitchListTile.adaptive(secondary: const Icon(Icons.music_note_rounded), title: const Text('Show in BEATS', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Add this video to the full-screen BEATS feed'), value: _isBeat, onChanged: _posting ? null : (value) => setState(() => _isBeat = value))),
      ...
      Card(child: Column(children: [
        ListTile(leading: const Icon(Icons.tune_rounded), title: const Text('Edit media', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Trim • Crop • Filter • Text • Audio'), trailing: const Icon(Icons.chevron_right_rounded), onTap: _posting ? null : _openTools),
        const Divider(height: 1),
        if (!_kidsContent) ListTile(leading: const Icon(Icons.people_outline_rounded), title: const Text('Audience'), subtitle: Text(_audience), trailing: const Icon(Icons.chevron_right_rounded), onTap: _posting ? null : _showAudience)
        else const ListTile(leading: Icon(Icons.child_care_rounded), title: Text('Kids audience'), subtitle: Text('Public in MANOX Kids only')),
        SwitchListTile.adaptive(secondary: const Icon(Icons.comment_outlined), title: const Text('Allow comments'), value: _comments, onChanged: _posting ? null : (value) => setState(() => _comments = value)),
      ...
      const SizedBox(height: 10),
      Card(child: SwitchListTile.adaptive(secondary: const Icon(Icons.child_care_rounded), title: const Text('Kids content', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Only appears in MANOX Kids'), value: _kidsContent, onChanged: widget.initialBeat || _posting ? null : (value) => setState(() { _kidsContent = value; if (value) { _isBeat = false; _audience = 'Everyone'; _downloads = false; } }))),
      ...
      const SizedBox(height: 8),
      OutlinedButton.icon(onPressed: _posting ? null : _openTools, icon: const Icon(Icons.auto_fix_high_rounded), label: const Text('More editing tools')),
    ])),
  );
