...
/// Video-first MANOX home surface inspired by familiar YouTube/Bilibili
/// navigation patterns, while keeping MANOX branding and creator flows.
...
class _YoutubeHomePageState extends State<YoutubeHomePage> {
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(
              ...
              child: Icon(Icons.change_history_rounded, color: Theme.of(context).colorScheme.surface),
            ),
            const SizedBox(width: 9),
            const Text('MANOX', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w900, letterSpacing: .7)),
            const Spacer(),
  ...
  Widget _creatorShortcuts() {
    const items = <({String title, IconData icon})>[
      (title: 'BEATS', icon: Icons.auto_awesome_rounded),
      (title: 'Live', icon: Icons.radio_rounded),
      (title: 'Trending', icon: Icons.local_fire_department_rounded),
      (title: 'Learn', icon: Icons.school_rounded),
      (title: 'Sports', icon: Icons.sports_soccer_rounded),
    ];
    return SizedBox(
      height: 74,
      child: ListView.separated(
        ...
        itemBuilder: (_, i) => InkWell(
          ...
          onTap: () {
            if (items[i].title == 'BEATS') context.push('/beats');
            else if (items[i].title == 'Live') context.push('/creator');
          },
          child: Container(
            width: 86,
            decoration: BoxDecoration(
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(items[i].title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
            ]),
          ),
        ),
      ),
  ...
  Widget _sectionHeader() {
    return Padding(
      ...
      child: Row(children: [
        Text(categories[_category], style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
        const Spacer(),
        IconButton(onPressed: _loadFeed, tooltip: 'Refresh', icon: const Icon(Icons.refresh_rounded)),
      ]),
    );
  }
  ...
  Widget _bottomNavigation() {
    return NavigationBar(
      ...
      onDestinationSelected: (i) {
        if (i == 0) setState(() => _bottom = 0);
        if (i == 1) { setState(() => _bottom = 1); context.push('/search'); }
        if (i == 2) { setState(() => _bottom = 0); _openCreate(); }
        if (i == 3) { setState(() => _bottom = 3); context.push('/notifications'); }
        if (i == 4) { setState(() => _bottom = 4); context.push('/profile'); }
      },
      destinations: const [
        ...
        child: ListView(
          ...
          children: [
            ...
            if (_loading)
              ...
              const Padding(padding: EdgeInsets.all(48), child: Center(child: Text('No content yet. Be the first creator.')))
            else
