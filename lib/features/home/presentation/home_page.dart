...
class _HomePageState extends State<HomePage> {
  ...
  @override
  void dispose() { _composerCtrl.dispose(); _feedScrollController.dispose(); super.dispose(); }
  ...
  void _openBeats() => _feedScrollController.animateTo(0, duration: const Duration(milliseconds: 350), curve: Curves.easeOut);
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Entertainment', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800)),
            const SizedBox(height: 14),
            Wrap(spacing: 8, runSpacing: 8, children: [
              for (final item in const [('Comedy', Icons.emoji_emotions_outlined), ('Movies', Icons.movie_outlined), ('Web Series', Icons.live_tv_outlined), ('Music', Icons.music_note_outlined), ('BEATS', Icons.auto_awesome_rounded)])
                ActionChip(avatar: Icon(item.$2, size: 19), label: Text(item.$1), onPressed: () { Navigator.pop(sheetContext); _showDiscovery(item.$1); }),
            ]),
          ]),
        ),
      ),
  ...
  void _showDiscovery(String label) => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$label discovery selected.')));

  Future<void> _submitPost() async {
    final text = _composerCtrl.text.trim();
    if (text.isEmpty && _selectedImagePath == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Add text or an image before posting.'))); return; }
    setState(() => _posting = true);
    try {
      String? imagePath;
      if (_selectedImagePath != null) { final picked = XFile(_selectedImagePath!); final bytes = await picked.readAsBytes(); imagePath = await _repository.uploadImage(bytes, picked.path.split('.').last.toLowerCase(), picked.mimeType); }
      final post = await _repository.createPost(text: text, imagePath: imagePath);
      if (!mounted) return;
      setState(() { _posts.insert(0, HomeDemoData(id: post.id, creatorName: post.creatorName, handle: post.handle, text: post.text, likes: 0, comments: 0, imagePath: post.imageUrl, isRemote: true, ownerUserId: post.ownerUserId)); _composerCtrl.clear(); _selectedImagePath = null; _posting = false; });
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Posted successfully.')));
    } catch (e) { if (!mounted) return; setState(() => _posting = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Post failed: ${e.toString().replaceFirst('Exception: ', '')}'))); }
  }
  ...
  void _openCreateSheet() {
    showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheetContext) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 24), child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      ]), const SizedBox(height: 12), FilledButton.icon(onPressed: _posting ? null : () => Navigator.pop(sheetContext), icon: const Icon(Icons.post_add_rounded), label: const Text('Open post composer')),
    ]))));
  }
