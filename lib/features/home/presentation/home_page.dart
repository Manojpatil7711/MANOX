...
class _HomePageState extends State<HomePage> {
  ...
  @override void dispose() { _composerCtrl.dispose(); _feedScrollController.dispose(); super.dispose(); }
  ...
  Future<void> _pickImage() async {
    ...
    if (image != null && mounted) setState(() { _selectedMediaPath = image.path; _selectedVideo = false; });
  }

  Future<void> _pickVideo(ImageSource source) async {
    try {
      final video = await _picker.pickVideo(source: source, maxDuration: const Duration(minutes: 10));
      if (video != null && mounted) setState(() { _selectedMediaPath = video.path; _selectedVideo = true; });
    } catch (e) { if (mounted) _showMessage('Video could not be selected: ${_cleanError(e)}'); }
  }
  ...
  Future<void> _chooseVideoSource() async {
    await showModalBottomSheet<void>(
      ...
      builder: (sheet) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ListTile(leading: const Icon(Icons.videocam_outlined), title: const Text('Record video'), onTap: () { Navigator.pop(sheet); _pickVideo(ImageSource.camera); }),
        ListTile(leading: const Icon(Icons.video_library_outlined), title: const Text('Choose from gallery'), onTap: () { Navigator.pop(sheet); _pickVideo(ImageSource.gallery); }),
      ])),
    );
  }
  ...
  Future<void> _openLiveCamera() async {
    try {
      ...
      setState(() { _selectedMediaPath = image.path; _selectedVideo = false; });
      _showMessage('Camera opened.');
    } catch (e) { if (mounted) _showMessage('Camera could not open: ${_cleanError(e)}'); }
  }
  ...
  Future<void> _submitPost() async {
    ...
    if (text.isEmpty && _selectedMediaPath == null) { _showMessage('Add text, image or video before posting.'); return; }
    ...
    try {
      ...
      if (_selectedMediaPath != null) {
        ...
        if (_selectedVideo) {
          mediaPath = await _repository.uploadVideo(bytes, extension, picked.mimeType);
          mediaType = 'video';
        } else {
          mediaPath = await _repository.uploadImage(bytes, extension, picked.mimeType);
          mediaType = 'image';
        }
      }
      final post = await _repository.createPost(text: text, imagePath: mediaPath, mediaType: mediaType);
      if (!mounted) return;
      setState(() {
        _posts.insert(0, HomeDemoData(id: post.id, creatorName: post.creatorName, handle: post.handle, text: post.text, imagePath: post.imageUrl, isRemote: true, ownerUserId: post.ownerUserId));
        _composerCtrl.clear();
  ...
  void _openEntertainment() {
    showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheetContext) {
      const items = <({String name, IconData icon})>[
        (name: 'Comedy', icon: Icons.emoji_emotions_outlined), (name: 'Movies', icon: Icons.movie_outlined),
        (name: 'Web Series', icon: Icons.live_tv_outlined), (name: 'Music', icon: Icons.music_note_outlined),
      ...
      return SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Entertainment', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800)), const SizedBox(height: 14),
        Wrap(spacing: 8, runSpacing: 8, children: items.map((item) => ActionChip(avatar: Icon(item.icon, size: 19), label: Text(item.name), onPressed: () { Navigator.pop(sheetContext); item.name == 'BEATS' ? _openBeats() : _showDiscovery(item.name); })).toList()),
      ])));
    });
  }
  ...
  void _openCreateSheet() {
    showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (sheetContext) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(18, 8, 18, 24), child: Column(mainAxisSize: MainAxisSize.min, children: [
      const Align(alignment: Alignment.centerLeft, child: Text('Create on MANOX', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800))), const SizedBox(height: 14),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: PreferredSize(preferredSize: const Size.fromHeight(kToolbarHeight), child: SafeArea(child: LayoutBuilder(builder: (context, constraints) {
      ...
      body: SafeArea(child: RefreshIndicator(onRefresh: _loadFeed, child: ListView(controller: _feedScrollController, physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.fromLTRB(12, 8, 12, 28), children: [
  ...
  Widget _discoveryRow(ThemeData theme) {
    const items = <({String label, IconData icon})>[(label: 'BEATS', icon: Icons.auto_awesome_rounded), (label: 'Live', icon: Icons.radio_rounded), (label: 'Trending', icon: Icons.local_fire_department_rounded), (label: 'Learn', icon: Icons.school_rounded), (label: 'Entertainment', icon: Icons.movie_rounded), (label: 'Sports', icon: Icons.sports_soccer_rounded)];
    return SizedBox(height: 78, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: items.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (_, index) {
      final item = items[index];
      return InkWell(borderRadius: BorderRadius.circular(16), onTap: () { if (item.label == 'BEATS') { _openBeats(); } else if (item.label == 'Live') { _openLiveCamera(); } else if (item.label == 'Entertainment') { _openEntertainment(); } else { _showDiscovery(item.label); } }, child: Container(width: 76, decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: theme.dividerColor)), padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 7), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(item.icon, size: 23), const SizedBox(height: 4), Text(item.label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700))])));
    }));
  }
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(children: [
      ...
      if (_selectedMediaPath != null) ...[
        ...
        if (_selectedVideo)
          ...
          ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(File(_selectedMediaPath!), height: 150, width: double.infinity, fit: BoxFit.cover)),
        if (_selectedVideo) const Align(alignment: Alignment.centerLeft, child: Padding(padding: EdgeInsets.only(top: 6), child: Text('VIDEO • ready to post', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700))),),
      ],
