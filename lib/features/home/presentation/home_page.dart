...
class _HomePageState extends State<HomePage> {
  ...
  void _openBeats() {
    _feedScrollController.animateTo(0, duration: const Duration(milliseconds: 350), curve: Curves.easeOut);
  }
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, index) => InkWell(
          ...
          onTap: () {
            if (items[index].$1 == 'BEATS') {
              _openBeats();
            } else {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${items[index].$1} discovery selected.')));
            }
          },
          child: Container(
            width: 76,
            ...
            child: Column(
              ...
              children: [
                Icon(items[index].$2, size: 23),
                const SizedBox(height: 4),
                Text(items[index].$1, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
        ),
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(child: TextField(key: const Key('post-composer-field'), controller: _composerCtrl, maxLines: 2, minLines: 1, decoration: const InputDecoration(hintText: 'Share a Beat…', border: InputBorder.none))),
                IconButton(tooltip: 'Add image', onPressed: _posting ? null : _pickImage, icon: const Icon(Icons.image_outlined)),
              ],
            ),
            if (_selectedImagePath != null) ...[
              const SizedBox(height: 8),
              ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.file(File(_selectedImagePath!), height: 150, width: double.infinity, fit: BoxFit.cover)),
            ],
            const Divider(height: 16),
            Row(
              children: [
                TextButton.icon(onPressed: _posting ? null : _openCreateSheet, icon: const Icon(Icons.add_circle_outline_rounded), label: const Text('Create')),
                const Spacer(),
                FilledButton(onPressed: _posting ? null : _submitPost, child: _posting ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Post')),
              ],
            ),
          ],
        ),
