...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadFeed() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _posts = remote.map((post) => HomeDemoData(id: post.id, creatorName: post.creatorName, handle: post.handle, text: post.text, likes: post.likes, comments: post.comments, imagePath: post.imageUrl, likedByMe: post.likedByMe, isRemote: true)).toList();
        _loadingFeed = false;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _discoveryRow(ThemeData theme) {
    const items = [('Live', Icons.radio_rounded), ('Trending', Icons.local_fire_department_rounded), ('Learn', Icons.school_rounded), ('Entertainment', Icons.movie_rounded), ('Sports', Icons.sports_soccer_rounded), ('BEATS', Icons.auto_awesome_rounded)];
    return SizedBox(height: 86, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: items.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (_, index) => InkWell(borderRadius: BorderRadius.circular(18), onTap: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${items[index].$1} discovery selected.'))), child: Container(width: 82, decoration: BoxDecoration(borderRadius: BorderRadius.circular(18), border: Border.all(color: theme.dividerColor)), padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 8), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(items[index].$2, size: 25), const SizedBox(height: 5), Text(items[index].$1, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700))]))));
  }
