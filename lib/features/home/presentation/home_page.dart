...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadFeed() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Live feed unavailable: ${e.toString().replaceFirst('Exception: ', '')}',
          ),
        ),
      );
    }
  ...
  Future<void> _openLiveCamera() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Camera opened. Add your Live details before starting.'),
        ),
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Camera could not open: ${e.toString().replaceFirst('Exception: ', '')}',
            ),
          ),
        );
      }
  ...
  Future<void> _submitPost() async {
    ...
    if (text.isEmpty && _selectedImagePath == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add text or an image before posting.')),
      );
      return;
    }
    ...
    try {
      ...
      if (_selectedImagePath != null) {
        ...
        imagePath = await _repository.uploadImage(
          bytes,
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _createAction(
                      ...
                      'Video',
                      () => _showComingSoon('Video upload'),
                    ),
                    _createAction(
                      sheetContext,
                      Icons.bolt_rounded,
                      'Shorts',
                      () => _showComingSoon('Shorts'),
                    ),
                    _createAction(
                      sheetContext,
                      Icons.school_outlined,
                      'Lecture',
                      () => _showComingSoon('Lecture'),
                    ),
                    _createAction(
                      sheetContext,
  ...
  void _showComingSoon(String feature) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('$feature is being prepared for MANOX.')),
    );
  }
  ...
  void _openMessages() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Messages are being connected to MANOX chat.')),
    );
  }
            ...
            children: [
              ...
              if (_loadingFeed)
                ...
                const Padding(
                  padding: EdgeInsets.all(36),
                  child: Center(
                    child: Text('No content yet. Be the first to create.'),
                  ),
                )
              else
                ..._posts.map(
        ...
        itemBuilder: (_, index) {
          return InkWell(
            ...
            onTap: () => ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('${items[index].$1} discovery selected.'),
              ),
            ),
            child: Container(
              width: 76,
