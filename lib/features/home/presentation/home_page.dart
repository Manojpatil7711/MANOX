...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadFeed() async {
    ...
    } catch (e) { if (!mounted) return; setState(() => _loadingFeed = false); _showMessage('Live feed unavailable: ${_cleanError(e)}'); }
  }
  ...
  void _openDiscovery(String label) {
    switch (label) {
      case 'Trending': context.push('/trending'); break;
      case 'Learn': context.push('/learn'); break;
      case 'Sports': context.push('/sports'); break;
      default: _showMessage('$label discovery selected.');
    }
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: RefreshIndicator(onRefresh: _loadFeed, child: ListView(controller: _feedScrollController, physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.fromLTRB(12, 8, 12, 28), children: [
        _discoveryRow(theme), const SizedBox(height: 12), _composerCard(), const SizedBox(height: 16),
