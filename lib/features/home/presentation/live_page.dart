...
class _LivePageState extends State<LivePage> {
  ...
  @override void initState() { super.initState(); _loadCreators(); }
  ...
  Future<void> _startLive() async {
    ...
    try {
      ...
      if (!mounted) return;
      if (clip != null) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Camera is ready. Real-time broadcast needs the MANOX streaming backend.')));
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(padding: const EdgeInsets.all(16), children: [
      Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        const SizedBox(height: 6), const Text('Start your creator broadcast from MANOX.', textAlign: TextAlign.center), const SizedBox(height: 18),
        ...
        const SizedBox(height: 8), const Text('Camera access is wired. Live multi-user streaming will be enabled when the streaming service is connected.', textAlign: TextAlign.center, style: TextStyle(fontSize: 12)),
      ]))),
      const SizedBox(height: 18), const Text('Creators', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)), const SizedBox(height: 8),
