...
class _LivePageState extends State<LivePage> {
  ...
  Future<void> _loadCreators() async {
    try {
      ...
      final seen = <String>{};
      final unique = posts.where((p) => p.ownerUserId != null && seen.add(p.ownerUserId!)).take(20).toList();
  ...
  Future<void> _startLive() async {
    ...
    try {
      ...
      if (!mounted) return;
      if (clip != null) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Camera is ready. Connect the MANOX streaming backend for live broadcast.')));
      }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        ...
        else if (_creators.isEmpty) const Padding(padding: EdgeInsets.all(24), child: Text('No creators available yet.'))
        else ..._creators.map((creator) => Card(child: ListTile(
...
class _LiveRoomSheetState extends State<_LiveRoomSheet> {
  ...
  static const List<double> _giftAmounts = <double>[0.05, 0.10, 0.25, 0.50, 1, 2, 5, 10, 25, 50, 100];
  ...
  Future<void> _openGiftPicker() async {
    final amount = await showModalBottomSheet<double>(
      context: context,
      showDragHandle: true,
      builder: (sheetContext) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 8, runSpacing: 8, children: _giftAmounts.map((value) => OutlinedButton.icon(
    ...
    final label = '₹${amount.toStringAsFixed(amount < 1 ? 2 : 0)}';
    setState(() => _selectedGift = label);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gift $label selected. Payment backend is required to send real money.')));
  }
  ...
  Widget build(BuildContext context) => SafeArea(child: Container(
    ...
    child: Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(16, 14, 10, 8), child: Row(children: [
      ...
      const Expanded(child: Center(child: Text('LIVE VIDEO\nStreaming backend required', textAlign: TextAlign.center, style: TextStyle(color: Colors.white54, fontSize: 18)))),
      Padding(padding: const EdgeInsets.fromLTRB(12, 8, 12, 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
        _LiveAction(icon: _liked ? Icons.favorite_rounded : Icons.favorite_border_rounded, label: '$_likes', onTap: _toggleLike),
      ...
      Padding(padding: EdgeInsets.fromLTRB(12, 4, 12, MediaQuery.of(context).viewInsets.bottom + 12), child: Row(children: [
        Expanded(child: TextField(
          controller: _commentController,
          style: const TextStyle(color: Colors.white),
          textInputAction: TextInputAction.send,
          onSubmitted: (_) => _sendComment(),
          decoration: InputDecoration(hintText: 'Write a comment…', hintStyle: const TextStyle(color: Colors.white54), filled: true, fillColor: Colors.white12, border: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide.none)),
        )),
        const SizedBox(width: 8),
        IconButton(onPressed: _sendComment, icon: const Icon(Icons.send_rounded, color: Colors.white)),
      ])),
      const Padding(padding: EdgeInsets.only(bottom: 10), child: Text('Creator earnings: ₹ 🔒  •  Monetization locked until payout/KYC is enabled.', style: TextStyle(color: Colors.white54, fontSize: 12))),
    ]),
  ));
}
...
class _LiveAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
    IconButton(onPressed: onTap, icon: Icon(icon, color: Colors.white), style: IconButton.styleFrom(backgroundColor: Colors.white12)),
