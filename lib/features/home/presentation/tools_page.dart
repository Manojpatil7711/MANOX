...
/// MANOX creator toolbox. It intentionally routes every visible tool to a
/// working flow instead of showing placeholder actions.
...
class _ToolsPageState extends State<ToolsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _openCreate, tooltip: 'Create post', icon: const Icon(Icons.add_box_outlined)),
        ],
      ),
      body: ListView(
        ...
        children: [
          ...
          _toolCard(
            icon: Icons.photo_edit_outlined,
            title: 'Photo Editor',
          ...
          const SizedBox(height: 20),
          Text(
            'The toolbox is designed around proven creator patterns: templates/design controls from Canva-style workflows and editing controls such as crop, trim, speed, filters, text and audio-oriented workflows from CapCut-style editors. MANOX only exposes controls that are implemented in the app; unsupported actions are not presented as working buttons.',
  ...
  Widget _toolCard({required IconData icon, required String title, required String subtitle, required VoidCallback onTap}) {
    return Card(
      ...
      child: ListTile(
        ...
        leading: Container(
          width: 46,
          height: 46,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: Theme.of(context).colorScheme.surfaceContainerHighest),
          child: Icon(icon),
        ),
