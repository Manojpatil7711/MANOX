...
class _BeatItemState extends State<_BeatItem> {
  ...
  Future<void> _resolve() async { final p = widget.post.imagePath; if (p == null || p.isEmpty) { if (mounted) setState(() => _loading = false); return; } try { final u = await widget.repository.signedMediaUrl(p); if (mounted) setState(() { _url = u; _loading = false; }); } catch (_) { if (mounted) setState(() => _loading = false); } }
  ...
  Future<void> _toggleLike() async {
    ...
    try { await widget.repository.toggleLike(widget.post.id, previous); }
    catch (_) { if (mounted) setState(() { _liked = previous; _likes += previous ? 1 : -1; }); }
  ...
  Future<void> _commentsSheet() async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (sheet) => SafeArea(child: Padding(padding: EdgeInsets.fromLTRB(16, 8, 16, MediaQuery.of(sheet).viewInsets.bottom + 16), child: StatefulBuilder(builder: (sheetContext, setSheetState) => SizedBox(height: MediaQuery.of(context).size.height * .65, child: Column(children: [
        const Align(alignment: Alignment.centerLeft, child: Text('Comments', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800))),
        const SizedBox(height: 10),
        Expanded(child: comments.isEmpty ? const Center(child: Text('No comments yet. Be the first.')) : ListView.separated(itemCount: comments.length, separatorBuilder: (_, __) => const Divider(height: 1), itemBuilder: (_, i) => ListTile(leading: const CircleAvatar(child: Icon(Icons.person_outline)), title: Text(comments[i].userName), subtitle: Text(comments[i].body)))),
        Row(children: [Expanded(child: TextField(controller: controller, textInputAction: TextInputAction.send, onSubmitted: (_) async { final body = controller.text.trim(); if (body.isEmpty) return; await widget.repository.addComment(widget.post.id, body); controller.clear(); _comments++; setState(() {}); setSheetState(() {}); }, decoration: const InputDecoration(hintText: 'Add a comment…'))), const SizedBox(width: 8), IconButton(icon: const Icon(Icons.send_rounded), onPressed: () async { final body = controller.text.trim(); if (body.isEmpty) return; await widget.repository.addComment(widget.post.id, body); controller.clear(); _comments++; setState(() {}); setSheetState(() {}); })]),
      ]))))),
    );
    controller.dispose();
    if (mounted) setState(() {});
  ...
  Future<void> _toggleSave() async {
    try {
      if (_saved) await widget.repository.unsaveContent(widget.post.id); else await widget.repository.saveContent(widget.post.id);
      if (mounted) setState(() => _saved = !_saved);
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not update Save: $e'))); }
  }
  ...
  Future<void> _share() async {
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('BEATS link copied. Share it anywhere.')));
    } catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not share this BEAT.'))); }
  }

  @override Widget build(BuildContext context) => Stack(fit: StackFit.expand, children: [
    if (_loading) const Center(child: CircularProgressIndicator(color: Colors.white)) else if (_url != null && isManoxVideo(widget.post.imagePath ?? '')) ManoxMediaPreview(url: _url!, height: double.infinity, fit: BoxFit.cover, autoPlay: true, loop: false, fullScreenStyle: true) else const _BeatFallback(),
    ...
    Positioned(right: 12, bottom: 26, child: Column(children: [
      _BeatAction(icon: _liked ? Icons.favorite_rounded : Icons.favorite_border_rounded, value: '$_likes', onTap: _toggleLike),
      const SizedBox(height: 18),
      _BeatAction(icon: Icons.mode_comment_outlined, value: '$_comments', onTap: _commentsSheet),
      const SizedBox(height: 18),
      _BeatAction(icon: _saved ? Icons.bookmark_rounded : Icons.bookmark_border_rounded, value: _saved ? 'Saved' : 'Save', onTap: _toggleSave),
      const SizedBox(height: 18),
      _BeatAction(icon: Icons.share_outlined, value: 'Share', onTap: _share),
    ])),
  ]);
}

class _BeatAction extends StatelessWidget {
  final IconData icon; final String value; final VoidCallback onTap;
  const _BeatAction({required this.icon, required this.value, required this.onTap});
