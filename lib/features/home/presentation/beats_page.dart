          ...
          child: Column(children: [
            ...
            Expanded(
              child: comments.isEmpty
                ...
                : ListView(children: comments.map((c) => ListTile(
                    title: Text(c.userName), subtitle: Text(c.body))).toList()),
            ),
            TextField(
              ...
              onSubmitted: (_) {
                ...
                if (body.isNotEmpty) {
                  widget.repository.addComment(widget.post.id, body);
                  controller.clear();
                }
              },
            ),
    ...
    try {
      ...
      if (mounted) {
        await SharePlus.instance.share(ShareParams(
          ...
          subject: 'Check out this BEAT on MANOX',
        ));
      }
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    return Stack(fit: StackFit.expand, children: [
      if (_loading)
        ...
        const _BeatFallback(),
      const IgnorePointer(child: DecoratedBox(
        decoration: BoxDecoration(gradient: LinearGradient(
          begin: Alignment.topCenter, end: Alignment.bottomCenter,
          colors: [Colors.transparent, Colors.black87], stops: [.55, 1],
        )),
      )),
      Positioned(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          InkWell(
            onTap: _openCreator,
            child: Text(widget.post.handle, style: const TextStyle(
              color: Colors.white, fontSize: 17, fontWeight: FontWeight.w800)),
          ),
          const SizedBox(height: 8),
          Text(widget.post.text, maxLines: 4,
      ...
      Positioned(
        ...
        child: Column(children: [
          _BeatAction(
            ...
            value: '$_likes', onTap: _toggleLike,
          ),
          const SizedBox(height: 18),
          _BeatAction(
            icon: _saved ? Icons.bookmark_rounded : Icons.bookmark_border_rounded,
            value: _saved ? 'Saved' : 'Save', onTap: _toggleSave,
          ),
          const SizedBox(height: 18),
          _BeatAction(icon: Icons.mode_comment_outlined,
            value: '$_comments', onTap: _commentsSheet),
          const SizedBox(height: 18),
          _BeatAction(icon: Icons.share_outlined, value: 'Share', onTap: _share),
          const SizedBox(height: 18),
          _BeatAction(icon: Icons.currency_rupee_rounded,
            value: '₹ 🔒', onTap: () => context.push('/monetization')),
        ]),
      ),
    ]);
  }
