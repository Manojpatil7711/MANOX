...
class _BeatItemState extends State<_BeatItem> {
                        ...
                        children: [
                          Expanded(
                            child: TextField(
                              ...
                              onSubmitted: (_) =>
                                  _submitComment(controller, setSheetState),
  ...
  Future<void> _share() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not share this BEAT.')),
        );
      }
    }
  }
  ...
  void _openMonetization() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('₹ Earnings are locked until monetization is enabled.'),
      ),
    );
      ...
      children: [
        if (_loading)
          const Center(child: CircularProgressIndicator(color: Colors.white))
            ...
            isManoxVideo(widget.post.imagePath ?? ''))
          ManoxMediaPreview(
            url: _url!,
