...
class _BeatItemState extends State<_BeatItem> {
  ...
  Future<void> _share() async { try { final url = await widget.repository.createShareUrl(widget.post.id); await Clipboard.setData(ClipboardData(text: url)); await widget.repository.recordShare(widget.post.id); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('BEATS link copied. Share it anywhere.'))); } catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not share this BEAT.'))); } }
