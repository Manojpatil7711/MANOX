...
class SupabasePostRepository {
  ...
  // BEATS is an explicit content type, not a generic video feed. This keeps
  // ordinary video posts out of BEATS unless the creator marks them as BEAT.
  Future<List<ManoxPost>> fetchBeats() async { final rows=await _client.from('contents').select(_contentSelect).eq('status','published').eq('visibility','public').eq('content_type','beat').order('published_at',ascending:false,nullsFirst:false).order('created_at',ascending:false).limit(50); return _mapRows(rows as List); }
