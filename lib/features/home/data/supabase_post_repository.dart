...
class ManoxPost { final String id; final String creatorName; final String handle; final String text; final int likes; final int comments; final String? imageUrl; final bool likedByMe; final String? ownerUserId; const ManoxPost({required this.id,required this.creatorName,required this.handle,required this.text,required this.likes,required this.comments,required this.likedByMe,this.imageUrl,this.ownerUserId}); }
...
class SupabasePostRepository {
  ...
  Future<List<ManoxPost>> _mapRows(List rows) async {
    ...
    return rows.map((row){
      ...
      return ManoxPost(id:row['id'] as String,creatorName:(profile?['display_name'] as String?)??'MANOX Creator',handle:'@${(profile?['username'] as String?)??'creator'}',text:(row['description'] as String?)??'',likes:likes.length,comments:comments.length,likedByMe:likes.any((like)=>like['user_id']==currentUser),imageUrl:mediaUrls.isNotEmpty?mediaUrls.first as String:row['media_url'] as String?,ownerUserId:row['owner_user_id'] as String?);
    }).toList();
  }
  Future<List<ManoxPost>> fetchFeed() async { final rows=await _client.from('contents').select('id, owner_user_id, description, media_url, media_urls, created_at, profiles!contents_owner_user_id_fkey(username, display_name), content_likes(user_id), content_comments(id)').eq('status','published').eq('visibility','public').order('published_at',ascending:false,nullsFirst:false).order('created_at',ascending:false).limit(50); return _mapRows(rows as List); }
  Future<List<ManoxPost>> fetchMyPosts() async { final rows=await _client.from('contents').select('id, owner_user_id, description, media_url, media_urls, created_at, profiles!contents_owner_user_id_fkey(username, display_name), content_likes(user_id), content_comments(id)').eq('owner_user_id',_userId).eq('status','published').order('created_at',ascending:false); return _mapRows(rows as List); }
  Future<String?> uploadImage(Uint8List bytes,String extension,String? mimeType) async {final path='$_userId/${DateTime.now().microsecondsSinceEpoch}.$extension';await _client.storage.from('manox-media').uploadBinary(path,bytes,fileOptions:FileOptions(contentType:mimeType,upsert:false,cacheControl:'3600'));return path;}
  Future<ManoxPost> createPost({required String text,String? imagePath}) async {final row=await _client.from('contents').insert({'owner_user_id':_userId,'content_type':imagePath==null?'post':'image','description':text,'media_url':imagePath,'media_urls':imagePath==null?<String>[]:<String>[imagePath],'status':'published','visibility':'public','published_at':DateTime.now().toIso8601String()}).select('id').single();final id=row['id'] as String;final post=await _client.from('contents').select('id, owner_user_id, description, media_url, media_urls, created_at, profiles!contents_owner_user_id_fkey(username, display_name)').eq('id',id).single();final profile=post['profiles'] as Map<String,dynamic>?;final urls=post['media_urls'] as List?;return ManoxPost(id:id,creatorName:(profile?['display_name'] as String?)??'You',handle:'@${(profile?['username'] as String?)??'you'}',text:(post['description'] as String?)??'',likes:0,comments:0,likedByMe:false,imageUrl:urls?.isNotEmpty==true?urls!.first as String:post['media_url'] as String?,ownerUserId:post['owner_user_id'] as String?);}
  ...
  Future<void> recordShare(String contentId,{String target='system'}) async {await _client.from('content_shares').insert({'content_id':contentId,'profile_id':_userId,'share_target':target});}
