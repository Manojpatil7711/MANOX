...
class ManoxPost { final String id,creatorName,handle,text; final int likes,comments; final String? imageUrl,avatarUrl,ownerUserId; final String contentType; final DateTime createdAt; final bool likedByMe,savedByMe,allowComments,allowDownloads; const ManoxPost({required this.id,required this.creatorName,required this.handle,required this.text,required this.likes,required this.comments,required this.likedByMe,required this.savedByMe,this.imageUrl,this.avatarUrl,this.contentType='post',this.ownerUserId,this.createdAt=DateTime.fromMillisecondsSinceEpoch(0),this.allowComments=true,this.allowDownloads=true}); }
...
class SupabasePostRepository {
  ...
  Future<List<ManoxPost>> fetchPostsByOwner(String ownerUserId) async {final rows=await _client.from('contents').select(_contentSelect).eq('owner_user_id',ownerUserId).eq('status','published').eq('audience_category','general').order('created_at',ascending:false);return _mapRows(rows as List);}
