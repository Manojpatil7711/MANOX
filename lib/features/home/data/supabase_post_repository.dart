...
class ManoxPost {
  ...
  const ManoxPost({required this.id, required this.creatorName, required this.handle, required this.text, required this.likes, required this.comments, required this.likedByMe, this.imageUrl, this.ownerUserId});
}
class ManoxComment {
  final String id;
  ...
  const ManoxComment({required this.id, required this.userName, required this.body, required this.createdAt});
}
class SupabasePostRepository {
  ...
  Future<ManoxPost> createPost({required String text, String? imagePath}) async {
    final row = await _client.from('contents').insert({'owner_user_id': _userId, 'content_type': imagePath == null ? 'post' : 'image', 'description': text, 'media_url': imagePath, 'media_urls': imagePath == null ? <String>[] : <String>[imagePath], 'status': 'published', 'visibility': 'public', 'published_at': DateTime.now().toIso8601String()}).select('id').single();
    ...
    final profile = post['profiles'] as Map<String, dynamic>?; final urls = post['media_urls'] as List?;
    return ManoxPost(id: id, creatorName: (profile?['display_name'] as String?) ?? 'You', handle: '@${(profile?['username'] as String?) ?? 'you'}', text: (post['description'] as String?) ?? '', likes: 0, comments: 0, likedByMe: false, imageUrl: urls?.isNotEmpty == true ? urls!.first as String : post['media_url'] as String?, ownerUserId: post['owner_user_id'] as String?);
  }
