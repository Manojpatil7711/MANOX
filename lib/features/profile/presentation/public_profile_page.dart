...
class _PublicProfilePageState extends State<PublicProfilePage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) {
      ...
      content = Center(child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.person_off_outlined, size: 48), const SizedBox(height: 10), Text(_error ?? 'Profile unavailable'), const SizedBox(height: 10), OutlinedButton(onPressed: _load, child: const Text('TRY AGAIN'))]));
    } else {
      final profile = _profile!;
      final flag = _flagForCountry(profile.countryCode);
      final profession = profile.profession?.trim() ?? '';
      final metadata = <Widget>[];
      if (profession.isNotEmpty) metadata.add(Text(profession, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500)));
      if (flag.isNotEmpty) metadata.add(Text(flag, style: const TextStyle(fontSize: 20)));
      if (profile.otherLink?.trim().isNotEmpty == true) metadata.add(InkWell(onTap: () => _openLink(profile.otherLink!.trim()), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.link_outlined, size: 18), SizedBox(width: 5), Text('Link', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))])));
      content = RefreshIndicator(onRefresh: _load, child: ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.fromLTRB(16, 18, 16, 32), children: [
        ...
        if (metadata.isNotEmpty) ...[const SizedBox(height: 8), ...metadata.map((w) => Padding(padding: const EdgeInsets.only(bottom: 3), child: w))],
        const SizedBox(height: 20),
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_Stat(value: '${_posts.length}', label: 'Posts'), _Stat(value: '${profile.followers}', label: 'Followers'), _Stat(value: '${profile.following}', label: 'Following')]),
