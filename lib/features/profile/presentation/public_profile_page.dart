...
class _PublicProfilePageState extends State<PublicProfilePage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      var following = false;
      if (!self) {
        try { following = await _postsRepo.isFollowing(widget.userId); } catch (_) {}
      }
      var followers = 0;
  ...
  HomeDemoData _data(ManoxPost post) => HomeDemoData(id: post.id, creatorName: post.creatorName, handle: post.handle, text: post.text, likes: post.likes, comments: post.comments, imagePath: post.imageUrl, likedByMe: post.likedByMe, isRemote: true, ownerUserId: post.ownerUserId);
  ...
  Widget _buildMetadata(ThemeData theme, ProfileData profile) {
    ...
    if (link.isNotEmpty) items.add(InkWell(onTap: () => _openLink(link), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.link_outlined, size: 18), SizedBox(width: 5), Text('Link', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))])));
      ...
      child: ListView(
        ...
        children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            CircleAvatar(radius: 46, backgroundImage: profile.avatarUrl == null ? null : NetworkImage(profile.avatarUrl!), child: profile.avatarUrl == null ? const Icon(Icons.person_outline_rounded, size: 42) : null),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (profile.isCreator) ...[const SizedBox(height: 7), Row(children: [Icon(Icons.verified_rounded, size: 16, color: theme.colorScheme.primary), const SizedBox(width: 5), const Text('CREATOR', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 0.8))])],
            ])),
          ]),
          if (!isSelf) ...[
            const SizedBox(height: 14),
            SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: _followBusy ? null : _toggleFollow, icon: _followBusy ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : Icon(_isFollowing ? Icons.person_remove_alt_1_rounded : Icons.person_add_alt_1_rounded), label: Text(_isFollowing ? 'Following' : 'Follow'))),
          ],
          if (profile.bio.trim().isNotEmpty) ...[const SizedBox(height: 10), Text(profile.bio, maxLines: 4, overflow: TextOverflow.ellipsis, style: theme.textTheme.bodyMedium?.copyWith(height: 1.35))],
          _buildMetadata(theme, profile),
          const SizedBox(height: 20),
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_Stat(value: '${_posts.length}', label: 'Posts'), _Stat(value: '$_followers', label: 'Followers'), _Stat(value: '$_following', label: 'Following')]),
          const SizedBox(height: 20),
          const Divider(height: 1),
          const SizedBox(height: 12),
          if (_posts.isEmpty) const Card(child: Padding(padding: EdgeInsets.all(28), child: Center(child: Text('No public posts yet.')))) else for (final post in _posts) Padding(padding: const EdgeInsets.only(bottom: 8), child: PostCard(data: _data(post), repository: _postsRepo, onChanged: _load)),
        ],
      ),
    );
  }
  ...
  Widget _buildBody(ThemeData theme) {
    ...
    if (_error != null || _profile == null) return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.person_off_outlined, size: 48), const SizedBox(height: 10), Text(_error ?? 'Profile unavailable'), const SizedBox(height: 10), OutlinedButton(onPressed: _load, child: const Text('TRY AGAIN'))]));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(leading: IconButton(onPressed: () => context.pop(), icon: const Icon(Icons.arrow_back_ios_new_rounded)), title: const Text('Profile')), body: SafeArea(child: _buildBody(theme)));
  }
}
