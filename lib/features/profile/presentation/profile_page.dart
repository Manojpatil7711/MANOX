...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void _openProfileSection(String label) {
    if (!mounted) return;
    if (label == 'Monetization') { context.push('/monetization'); return; }
    if (label == 'Wallet') { context.push('/payout'); return; }
  ...
  String _countryLabel(String? code) => code == null || code.isEmpty ? '' : code.toUpperCase();
  String _genderLabel(String? value) {
    switch (value) { case 'female': return 'Female'; case 'male': return 'Male'; case 'other': return 'Other'; default: return ''; }
  }
  ...
  Widget _profileDetails(ProfileData profile) {
    ...
    if ((profile.profession ?? '').trim().isNotEmpty) details.add(profile.profession!.trim());
    if (details.isEmpty) return const SizedBox.shrink();
    return Card(margin: EdgeInsets.zero, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), child: Wrap(spacing: 8, runSpacing: 8, children: details.map((text) => Chip(label: Text(text))).toList())));
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) body = const Center(child: CircularProgressIndicator());
      ...
      final profile = _profile!;
      body = RefreshIndicator(onRefresh: _load, child: ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.fromLTRB(16, 16, 16, 32), children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(profile.displayName, key: const Key('profile-name'), maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.headlineSmall?.copyWith(fontSize: 23, fontWeight: FontWeight.w800, letterSpacing: -0.35, height: 1.1)),
            const SizedBox(height: 5),
            Text(profile.handle, key: const Key('profile-handle'), maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.bodyMedium?.copyWith(fontSize: 14, fontWeight: FontWeight.w600, letterSpacing: 0.15)),
            if (profile.isCreator) ...[const SizedBox(height: 8), Row(children: [Icon(Icons.verified_rounded, size: 16, color: theme.colorScheme.primary), const SizedBox(width: 5), const Text('CREATOR', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 0.8))])],
          ])),
        ]),
        ...
        Row(children: [Expanded(child: OutlinedButton.icon(key: const Key('profile-edit-button'), onPressed: _editProfile, icon: const Icon(Icons.edit_outlined, size: 18), label: const Text('EDIT PROFILE'))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(key: const Key('profile-share-button'), onPressed: _shareProfile, icon: const Icon(Icons.ios_share_outlined, size: 18), label: const Text('SHARE')))]),
        const SizedBox(height: 18),
        Card(margin: EdgeInsets.zero, child: Padding(padding: const EdgeInsets.symmetric(vertical: 15), child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_Stat(value: '${profile.postIds.length}', label: 'Posts'), _Stat(value: '${profile.followers}', label: 'Followers'), _Stat(value: '${profile.following}', label: 'Following')]))),
        const SizedBox(height: 14),
        ...
        Container(height: 1, color: theme.dividerColor), const SizedBox(height: 12),
        if (_selectedTab == 0) if (_posts.isEmpty) const _EmptySection(message: 'Your posts will appear here.') else ..._posts.map((post) => Padding(padding: const EdgeInsets.only(bottom: 8), child: PostCard(data: _toHomePost(post), repository: _postRepo, onChanged: _load))) else _EmptySection(message: _selectedTab == 1 ? 'Your BEATS will appear here.' : 'Your media will appear here.'),
      ]));
    }
...
class _ProfileAction extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Card(margin: EdgeInsets.zero, child: InkWell(borderRadius: BorderRadius.circular(12), onTap: onTap, child: Padding(padding: const EdgeInsets.all(13), child: Row(children: [Icon(icon, size: 24), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 3), Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall)]))])));
}
...
class _ProfileTab extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final theme = Theme.of(context); return Expanded(child: InkWell(onTap: onTap, child: Padding(padding: const EdgeInsets.only(bottom: 10), child: Column(children: [Icon(icon, size: 21, color: selected ? theme.colorScheme.primary : theme.colorScheme.onSurfaceVariant), const SizedBox(height: 5), Text(label, style: theme.textTheme.labelSmall?.copyWith(fontWeight: FontWeight.w700, letterSpacing: 0.5, color: selected ? theme.colorScheme.primary : null))]))); }
}

class _EmptySection extends StatelessWidget {
  final String message;
  const _EmptySection({required this.message});
  @override Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(28), child: Column(children: [const Icon(Icons.inbox_outlined, size: 42), const SizedBox(height: 8), Text(message)])));
}
