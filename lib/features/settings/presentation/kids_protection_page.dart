...
class _KidsProtectionPageState extends State<KidsProtectionPage> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _parentPasswordSet =
              user.userMetadata?['kids_parent_password_set'] == true;
        });
      }
    } catch (_) {
      if (mounted) {
  ...
  Future<void> _toggle(bool value) async {
    ...
    // First-time password setup must be a separate step. Do not continue
    // into the toggle/navigation flow after the dialog closes. This prevents
    // the password dialog from unexpectedly sending the user to Kids Home.
    if (value && !_parentPasswordSet) {
      final created = await _setParentPassword(firstSetup: true);
      if (!created || !mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Parent Password saved. Turn on Kids Protection to continue.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Kids Protection OFF • Main flow unlocked')),
      );
      context.go('/home');
    } catch (_) {
      if (mounted) {
    ...
    final password = await showDialog<String>(
      ...
      builder: (dialog) => AlertDialog(
        ...
        content: TextField(
          ...
          obscureText: true,
          autofocus: true,
          decoration: const InputDecoration(labelText: 'Parent/account password'),
        ),
        actions: [
          TextButton(
  ...
  Future<bool> _setParentPassword({required bool firstSetup}) async {
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              obscureText: true,
              autofocus: true,
              decoration: const InputDecoration(labelText: 'New parent password'),
            ),
            ...
            const Align(
              ...
              child: Text('Use at least 8 characters. This becomes the MANOX account password.'),
            ),
          ],
        ),
        actions: [
          ...
          FilledButton(
            onPressed: () => Navigator.pop(dialog, [controller.text, confirmController.text]),
    ...
    if (newPassword.length < 8 || newPassword != result[1]) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Password must be at least 8 characters and both entries must match.')),
        );
      }
      return false;
    }
    ...
    try {
      ...
      await client.auth.updateUser(
        UserAttributes(
          ...
          data: {'kids_parent_password_set': true},
        ),
      );
      if (!mounted) return true;
      setState(() => _parentPasswordSet = true);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Parent Password configured securely.')),
      );
      return true;
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not update Parent Password.')),
        );
      }
      return false;
    }
  ...
  Future<void> _changeParentPassword() async {
    await _setParentPassword(firstSetup: !_parentPasswordSet);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Kids Protection', style: TextStyle(fontWeight: FontWeight.w800)),
      ),
      body: ListView(
        ...
        children: [
          Card(
            child: SwitchListTile.adaptive(
              ...
              subtitle: Text(
                _enabled
                    ? 'ON • Kids flow active and Main flow locked'
                    : 'OFF • Set Parent Password before enabling',
              ),
              value: _enabled,
          ...
          Card(
            child: ListTile(
              ...
              subtitle: Text(
                _parentPasswordSet
                    ? 'Required to unlock Kids Protection and return to the main flow'
                    : 'Required before Kids Protection can be turned ON',
              ),
          ...
          const SizedBox(height: 8),
          const Text(
            'Only Science Experiments, History, Geography Knowledge, GK and Cartoon are exposed in Kids mode. Adult feed, BEATS, monetization, withdrawal and messaging stay locked.',
