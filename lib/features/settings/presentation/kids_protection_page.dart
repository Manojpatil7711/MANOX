...
class _KidsProtectionPageState extends State<KidsProtectionPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { _enabled = row?['enabled'] == true; _parentPasswordSet = user.userMetadata?['kids_parent_password_set'] == true; });
    } catch (_) {
      if (mounted) setState(() { _enabled = false; _parentPasswordSet = user.userMetadata?['kids_parent_password_set'] == true; });
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
