...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _loadPrivacy() async {
    ...
    try {
      ...
      // Keep safe local defaults when the privacy row is unavailable.
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }
  ...
  Future<void> _savePrivacy() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not save privacy settings.')));
      }
    } finally {
  ...
  Future<void> _openSecurity() async {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text('Reset your password through the secure MANOX recovery flow.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('CLOSE')),
          FilledButton(onPressed: () { Navigator.pop(dialogContext); context.push('/auth/forgot'); }, child: const Text('RESET PASSWORD')),
        ],
      ),
    );
  }
