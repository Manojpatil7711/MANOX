...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _loadPrivacy() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Privacy settings could not be loaded.')));
  ...
  Future<void> _savePrivacy() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not save privacy settings. Please try again.')));
  ...
  void _showKidsSafety() {
    showModalBottomSheet<void>(context: context, isScrollControlled: true, showDragHandle: true, builder: (sheetContext) {
      ...
      return StatefulBuilder(builder: (context, setSheetState) => SafeArea(child: Padding(
        padding: const EdgeInsets.fromLTRB(20, 4, 20, 24),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Row(children: [Icon(Icons.child_care_rounded, size: 28), SizedBox(width: 10), Text('Kids & Safety', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w800))]),
          const SizedBox(height: 8),
          const Text('A safer experience for younger users. MANOX keeps privacy, messaging and discovery restrictions stricter in Kids mode.'),
          const SizedBox(height: 12),
          SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('Kids safety mode'), subtitle: const Text('Keep age-appropriate safety protections enabled.'), value: safeMode, onChanged: (value) => setSheetState(() => safeMode = value)),
          SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('Restrict direct messages'), subtitle: const Text('Recommended for younger users.'), value: restrictMessages, onChanged: (value) => setSheetState(() => restrictMessages = value)),
          const SizedBox(height: 8),
          const Text('Safety note: Kids mode does not unlock monetization or withdrawal features.', style: TextStyle(fontWeight: FontWeight.w600)),
        ]),
      )));
    });
  }
  ...
  ListTile _item({required IconData icon, required String title, String? subtitle, VoidCallback? onTap, Widget? trailing}) => ListTile(leading: Icon(icon), title: Text(title), subtitle: subtitle == null ? null : Text(subtitle), trailing: trailing ?? (onTap == null ? null : const Icon(Icons.chevron_right_rounded)), onTap: onTap);
    ...
    return Scaffold(
      ...
      body: _loading ? const Center(child: CircularProgressIndicator()) : ListView(padding: const EdgeInsets.fromLTRB(12, 4, 12, 32), children: [
        ...
        _section('Privacy', Icons.shield_outlined, [
          SwitchListTile(secondary: const Icon(Icons.lock_person_outlined), title: const Text('Private account'), subtitle: const Text('Control who can discover and interact with you.'), value: _privateAccount, onChanged: (v) => setState(() => _privateAccount = v)),
        ...
        _section('Messages & Contacts', Icons.chat_bubble_outline_rounded, [
          ...
          SwitchListTile(secondary: const Icon(Icons.contact_page_outlined), title: const Text('Allow contact sharing'), subtitle: const Text('Contact details stay hidden unless you explicitly share them.'), value: _allowContactSharing, onChanged: (v) => setState(() => _allowContactSharing = v)),
          _item(icon: Icons.block_outlined, title: 'Blocked accounts', subtitle: 'Manage people you have blocked'),
        ]),
        _section('Kids & Safety', Icons.child_care_rounded, [_item(icon: Icons.verified_user_outlined, title: 'Kids safety', subtitle: 'Age-appropriate privacy, discovery and messaging protections', onTap: _showKidsSafety)]),
        _section('Notifications', Icons.notifications_none_rounded, [SwitchListTile(secondary: const Icon(Icons.notifications_active_outlined), title: const Text('Notifications'), subtitle: const Text('Likes, comments, VIBEs and messages'), value: _notifications, onChanged: (v) => setState(() => _notifications = v))]),
