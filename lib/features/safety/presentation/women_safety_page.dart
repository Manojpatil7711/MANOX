...
class _WomenSafetyPageState extends State<WomenSafetyPage> {
  ...
  Future<void> _send() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: <Widget>[
          ...
          FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: Text(firstAlert ? 'SEND ALERT' : 'ESCALATE')),
        ],
      ),
    );
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(firstAlert ? 'Safety Alert sent.' : 'Emergency alert escalated.')));
        ...
        children: <Widget>[
          ...
          const Card(
            child: ListTile(
              ...
              subtitle: Text('Exact location is not shared until an alert is confirmed. Safety access is controlled by the verified profile gender field.'),
            ),
          ),
        ],
      ),
