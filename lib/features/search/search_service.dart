...
class ManoxSearchService {
  ...
  Future<List<ManoxSearchResult>> search(String rawQuery) async {
    ...
    final byUsername = await client
        .from('profiles')
        .select('id, username, display_name, profession, country_code')
        .ilike('username', pattern)
        .limit(20);
    final byDisplayName = await client
        .from('profiles')
        .select('id, username, display_name, profession, country_code')
        .ilike('display_name', pattern)
        .limit(20);

    for (final row in [...(byUsername as List), ...(byDisplayName as List)]) {
      final id = row['id'] as String?;
      if (id == null || !seenProfiles.add(id)) continue;
      final name = (row['display_name'] as String?)?.trim();
      final username = (row['username'] as String?)?.trim();
      final profession = (row['profession'] as String?)?.trim();
      final fallbackUsername = username?.isNotEmpty == true ? username! : 'user';
      results.add(ManoxSearchResult(
        id: id,
        title: name?.isNotEmpty == true ? name! : '@$fallbackUsername',
        subtitle: profession?.isNotEmpty == true ? profession! : '@$fallbackUsername',
        profile: true,
      ));
    }
    ...
    for (final row in (posts as List)) {
      ...
      results.add(ManoxSearchResult(
        id: id,
        title: description.isEmpty ? 'MANOX ${row['content_type'] ?? 'content'}' : description,
        subtitle: 'Content',
        profile: false,
      ));
    }
